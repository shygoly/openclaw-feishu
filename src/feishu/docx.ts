...
async function clearDocumentContent(client: Lark.Client, docToken: string) {
  ...
  const childIds =
    existing.data?.items
      ?.filter(
        (b: { parent_id?: string; block_type?: number }) =>
          b.parent_id === docToken && b.block_type !== 1,
      )
...
async function listFolder(client: Lark.Client, folderToken: string) {
  ...
  return {
    files: res.data?.files?.map(
      (f: { token?: string; name?: string; type?: string; url?: string }) => ({
        token: f.token,
        name: f.name,
        type: f.type,
        url: f.url,
...
async function listAppScopes(client: Lark.Client) {
  ...
  return {
    granted: granted.map((s: { scope_name?: string; scope_type?: string }) => ({
      name: s.scope_name,
      type: s.scope_type,
    })),
    pending: pending.map((s: { scope_name?: string; scope_type?: string }) => ({
      name: s.scope_name,
      type: s.scope_type,
    })),
