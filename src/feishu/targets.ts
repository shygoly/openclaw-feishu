...
export function normalizeFeishuTarget(raw: string): string | undefined {
  const trimmed = raw.trim();
  if (!trimmed) return undefined;

  const lowered = trimmed.toLowerCase();
  if (lowered.startsWith("chat:")) {
    return trimmed.slice("chat:".length).trim() || undefined;
  }
  if (lowered.startsWith("user:")) {
    return trimmed.slice("user:".length).trim() || undefined;
  }
  if (lowered.startsWith("open_id:")) {
    return trimmed.slice("open_id:".length).trim() || undefined;
  }
