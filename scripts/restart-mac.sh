...
run_step "clean build cache" bash -lc "cd '${ROOT_DIR}/apps/macos' && rm -rf .build-swift 2>/dev/null || true && if [[ -d .build ]]; then find .build -mindepth 1 -maxdepth 1 -not -name checkouts -not -name repositories -exec rm -rf {} +; fi"
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product OpenClaw --skip-update"
